...
**IDOR — `.pcap` file exposed at `/data/0`:**
...
Navigating to `/data/0` retrieves a capture that was not generated by the current session. Opening it in Wireshark discloses plaintext FTP credentials.
...
The user flag is accessible via FTP. The same credentials are also valid for SSH.
