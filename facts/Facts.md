...
Facts is an easy Linux machine hosting a Camaleon CMS instance vulnerable to an authenticated arbitrary file read (CVE-2024-46987). After registering an account to an exposed admin endpoint, the CVE is leveraged to read local files. The site also allows to change the role parameter through a password change request, granting full admin privileges on the application. 
